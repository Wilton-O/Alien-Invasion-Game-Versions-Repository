...
class AlienInvasion:
    ...
    def _check_play_button(self, mouse_pos):
        ...
        if button_clicked and not self.game_active:
            ...
            self.sb.prep_score()                        # reset game score
            self.sb.prep_level()                        # reset game level
            self.sb.prep_ships()                        # reset ships left
            self.game_active = True                     # reset game status

            self.bullets.empty()                        # reset bullets
            self.aliens.empty()                         # reset alien fleet

            self._create_fleet()                        # create new fleet
            self.ship.center_ship()                     # create new ship
    ...
    def _ship_hit(self):
        ...
        ship_death_sound = pygame.mixer.Sound('sounds/ship-death.wav')

        if self.stats.ships_left > 0:
            print("Ship Hit!!!")
            
            ship_death_sound.play(0)
    ...
    def _check_bullet_alien_collisions(self):
        ...
        # Play sound, Add points for every alien hit by a bullet
        if collisions:
            alien_death_sound = pygame.mixer.Sound('sounds/alien-death.wav')
            alien_death_sound.play(0)
        ...
        if not self.aliens:
            ...
            # Also induce 'per-round' game speed-up
            self.settings.increase_speed()

            # Also increase 'round' level
            self.stats.level += 1
            self.sb.prep_level()
    ...
    def _create_fleet(self):
        ...
        aliens_per_row = 7
        current_y = 100     # Changed property from prev. ver. to start at 100 px., so not to collide with scoreboard
